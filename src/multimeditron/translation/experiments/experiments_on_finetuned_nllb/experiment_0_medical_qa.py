"""
Experiment 0: Does Meditron Need Translation?

Compares translation pipeline vs native multilingual approach for medical QA.
Tests on MediBench dataset across high-resource languages.

Output: src/multimeditron/translation/experiments/results/base_nllb/experiment_0_results.json
"""
...
def load_medibench_dataset():
    """Load MediBench dataset with fallback for inconsistent format."""
        ...
        with jsonlines.open(file_path) as reader:
            for obj in reader:
                if 'answer' in obj and isinstance(obj['answer'], list):
                    obj['answer'] = obj['answer'][0] if obj['answer'] else ''
                fixed_data.append(obj)
        
        print(f"Loaded {len(fixed_data)} samples manually")
        return Dataset.from_list(fixed_data)


def explore_dataset(dataset, num_samples=5):
    """Show dataset structure and get user confirmation to proceed."""
    print("\n" + "="*70)
...
class MedicalQAEvaluator:
    ...
    def print_summary(self):
        ...
        print("\n" + "="*70)
        print("EXPERIMENT 0: DOES MEDITRON NEED TRANSLATION?")
        print("="*70)
        ...
        if trans.get('by_language'):
            ...
            for lang, data in sorted(trans['by_language'].items(), key=lambda x: -x[1]['total']):
                print(f"      {lang:5s}: {data['accuracy']:5.1f}% ({data['correct']:3d}/{data['total']:3d})")
        ...
        if trans['total'] > 0 and native['total'] > 0:
            ...
            for lang, lang_diff, count in sorted(lang_diffs, key=lambda x: -x[2]):
                if count > 0:
                    ...
                    print(f"      {symbol} {lang:5s}: {lang_diff:+6.1f}% (n={count})")
    ...
    def save_results(self, output_path: str):
        ...
        output = {
            'experiment': 'Experiment 0: Does Meditron Need Translation?',
...
def run_experiment(args):
    """Run Experiment 0: Translation vs Native comparison."""
    print("\n[1/6] Loading MediBench dataset...")
    dataset = load_medibench_dataset()
    ...
    print("\n   Filtering to high-resource languages only (excluding English):")
    filtered_by_lang = {
        lang: samples
    ...
    for lang, samples in filtered_by_lang.items():
    ...
    for lang, samples in filtered_by_lang.items():
        ...
        print(f"   Selected {take} samples from {lang}")
    ...
    for idx, sample in enumerate(tqdm(valid_samples, desc="Processing")):
        ...
        if isinstance(raw_q, list):
            question = " ".join([str(x) for x in raw_q])
        else:
            question = str(raw_q)

        raw_opts = sample['options']
        options = [str(x) for x in raw_opts]

        correct_answer = sample['answer']
        if isinstance(correct_answer, list):
            correct_answer = str(correct_answer[0]).strip().upper()
        else:
        ...
        try:
            ISO_TO_NLLB = {
                ...
                'fr': 'fra_Latn', 'ja': 'jpn_Jpan', 'ru': 'rus_Cyrl',
                'xh': 'xho_Latn', 'rw': 'kin_Latn', 'ha': 'hau_Latn', 'ln': 'lin_Latn',
            }
            
            nllb_lang = ISO_TO_NLLB.get(language, 'eng_Latn')
            ...
            CHINESE_VARIANTS = {'zho_Hans', 'zho_Hant', 'yue_Hant', 'yue_Hans'}
            detected_lang_normalized = 'zho_Hans' if detected_lang_ft in CHINESE_VARIANTS else detected_lang_ft
            nllb_normalized = 'zho_Hans' if nllb_lang in CHINESE_VARIANTS else nllb_lang
            
            evaluator.track_fasttext(nllb_normalized, detected_lang_normalized, confidence, question)
            
            # Translation pipeline
            english_question = translator.translate_to_english(question)
            english_options = [translator.translate_to_english(opt) for opt in options]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Experiment 0: Does Meditron Need Translation?"
    )
    parser.add_argument(
        "--max_samples", 
        type=int, 
        default=None,
        help="Maximum samples to process (default: all valid samples)"
    )
    parser.add_argument(
        "--output",
        default="src/multimeditron/translation/experiments/results/finetuned_nllb/experiment_0_results.json",
